        ...
        <div className="location-selector">
          <select value={selectedLocation || ''} onChange={(e) => handleLocationChange(e.target.value)}>
            ...
            {uniqueLocations.map((loc) => (
              <option key={loc} value={loc}>{loc} ({allCounters.locations[loc] || 0})</option>
            ))}
          </select>
        </div>
        <div className="event-buttons">
          <button onClick={() => handleEventClick('Nikkah')} className={selectedEvent === 'Nikkah' ? 'active' : ''}>Nikkah</button>
          <button onClick={() => handleEventClick('Shalima')} className={selectedEvent === 'Shalima' ? 'active' : ''}>Shalima</button>
          <button onClick={() => handleEventClick(null)} className={selectedEvent === null ? 'active' : ''}>All Events</button>
        </div>
        <div className="counter-grid">
          <div className="counter-item"><span className="counter-number">{displayedCounters.nikkah}</span><span className="counter-label">Nikkah Attendees</span></div>
          <div className="counter-item"><span className="counter-number">{displayedCounters.shalima}</span><span className="counter-label">Shalima Attendees</span></div>
          <div className="counter-item"><span className="counter-number">{displayedCounters.totalMales}</span><span className="counter-label">Total Males</span></div>
          <div className="counter-item"><span className="counter-number">{displayedCounters.totalFemales}</span><span className="counter-label">Total Females</span></div>
          <div className="counter-item total"><span className="counter-number">{displayedCounters.totalGuests}</span><span className="counter-label">Total Attendees</span></div>
        </div>
      </div>
